    ...
    public void process(MetaData metaData, @Nullable MappingMetaData mappingMd, boolean allowIdGeneration, String concreteIndex) {
        ...
        if (mappingMd != null) {
            ...
            if (parseContext.shouldParse()) {
                ...
                try {
                    // 获取XContentParser解析器，其中还会进行压缩处理
        ...
        if (allowIdGeneration) {
            // TODO 处理ID，如果自动生成ID，判断ID是否由客户端已经提供，如果没有则使用Strings.base64UUID()生成，且opType标识为CREATE
            if (id == null) {
        ...
        if (timestamp == null) {
            // TODO 处理timestamp，如果没有provided，就generate
